...
export default {
  ...
  created() {
    ...
    this.mainEventId = this.$route.query.mainEventId
      ...
      : 0;
    // this.selected = this.$route.query.eventId
  ...
  methods: {
    ...
    setTypes() {
      ...
      window
        ...
        .each(function () {
          ...
          if (
            window.$(this).children("input").val() ===
            guestData.appUser.address.locationType
          ) {
            window.$(this).addClass("active");
          }
        });
    },
    openDetails(tabIndex, newTab, oldTab) {
      console.log(newTab.$options._parentVnode.data.key);
      const eventId = newTab.$options._parentVnode.data.key;
      this.isActiveClass = true;
      axios
        .get("http://localhost:" + this.port + "/events/sub/" + eventId)
        .then((res) => {
          // console.log(this.preFilledEventData);
          this.subeventDetails = res.data;
          console.log(this.subeventDetails);
    ...
    changeRSVPStatus(index, status) {
      this.invites[index].rsvpStatus = status;

      console.log(this.invites);
    },
    activeClass(index) {
      if (this.invites[index].rsvpStatus === "ATTENDING") {
        return "btn active";
      } else {
        return "btn";
      }
    },
    disableInput(rsvpStatus) {
      return rsvpStatus === "NOTATTENDING" ? true : false;
    },
    rsvpSubmit() {
      this.invites.forEach((invite) => {
        if (invite.rsvpStatus === "NOTATTENDING") {
          invite.actualGuests = 0;
        }
      });
      ...
      axios
        ...
        .then(() => {
          window.toastr.success("RSVP was Successfully");
          return false;
        })
        .catch(() => {
          window.toastr.error("RSVP Failed!");
        });
    },
    editSubevent() {
      this.$router.push({
